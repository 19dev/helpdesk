		...
		def tickets_list_table(tickets=@tickets, options={})
			content = content_tag :div, class: "row-fluid", id: "tickets_list_div" do
				table_for tickets, html: {class: "table table-striped table-bordered", id: "tickets_list_table"} do
					column title: t("tickets.label.reference"), html: { th: { class: "span1" }  } do |ticket|
				  	link_to ticket.reference, helpdesk.ticket_path(ticket)
				  end
				  column title: t("tickets.label.title"), html: { th: { class: "span3" }  } do |ticket|
				  	link_to ticket.title, helpdesk.ticket_path(ticket)
				  end
				  column title: t("tickets.label.user"), html: { th: { class: "span2" } } do |ticket|
				  ...
				  column title: t("tickets.label.status"), html: { th: { class: "span1" } } do |ticket|
				  	t("tickets.status.#{ticket.status}")
				  end
				  column title: t("tickets.label.assigned"), html: { th: { class: "span2" } } do |ticket|
          ...
          column :close_date, title: t("tickets.label.close_date"), html: { th: { class: "span2" } }
				end
			end
